# 🚢 Battleboats 2026 

Voici quelques ressources aind de vous simplifier le codage. 

>  📁 PoC Full

Contient un PoC sur comment interfacer tous les élements nécéssaires à la course: Télécommande, récepteur, encodeur, Cube Orange, Raspberry Pi et les servos.

>  📁 PoC Servos

Contient un PoC sur comment interfacer le Cube Orange, Raspberry Pi et les servos. Plus simple mais pas suffisant pour la course (éducatif).

Dans le wiki, vous trouverez les mises à jour et l'opportunité de poser des questions. 


## 🏆 Que les melleurs gagnent !

Compétition de voiliers autonomes · Toulon, Fort Saint-Louis · **9-10 mai 2026**
